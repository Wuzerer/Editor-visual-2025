...
init python:
    ...
    def save_project():
        ...
        try:
            ...
            project_name = "proyecto_visual_editor"
            success = save_project_rpy(project_name, scenes)
            if success:
                renpy.notify(f"✅ Proyecto '{project_name}' guardado exitosamente")
            else:
                renpy.notify("❌ Error al guardar el proyecto")
        except Exception as e:
    ...
    def load_project():
        ...
        try:
            ...
            project_name = "proyecto_visual_editor"
            loaded_scenes = load_project_rpy(project_name)
            if loaded_scenes:
                renpy.set_screen_variable("current_scenes", loaded_scenes)
                renpy.notify(f"✅ Proyecto '{project_name}' cargado: {len(loaded_scenes)} escenas")
            else:
                renpy.notify("❌ No se encontró el proyecto")
        except Exception as e:
            renpy.notify(f"❌ Error: {str(e)}")
